...
public class ticket_before_pay extends JFrame{
	...
	JLabel point,show_point,pay,show_time,show_name,show_theater,show_date,show_room,show_peo1,show_peo2,show_seat,show_price3;
	//포인트와 포인트표시 라벨, 각 영화이름, 날짜, 극장, 인원, 좌석, 금액등은 앞의 매표소에서 선택한 것들이 넘어올수있도록 출력하자.
	JButton pay_bt, back_bt;
	
	ticket_seat seatWindow;
	ImageIcon selectedMovie;
	String selectedDate,selectedTime,selectedTheater,selectedSeat, 
	selectedPrice;
	 
	public ticket_before_pay(ticket_seat seatWindow,ImageIcon moviePoster , String selectedDate, String selectedTime, 
			String selectedPeo1,String selectedPeo2,String selectedTheater, String selectedSeat, String selectedPrice) {
		super("결제확인창");
		this.seatWindow = seatWindow;
        this.selectedMovie = moviePoster;
	    ...
	    // 예시: 극장 설정
	    show_theater.setText(selectedTheater);
	    show_peo1.setText(selectedPeo1);
	    show_peo2.setText(selectedPeo2);
	    show_theater.setText(selectedTheater);
	    
	    // 예시: 좌석 설정
	    show_seat.setText(selectedSeat);
	    
	    // 예시: 가격 설정
	    show_price3.setText(selectedPrice);
