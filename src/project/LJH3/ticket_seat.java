...
public class ticket_seat extends JFrame {
	...
	public ticket_seat() {
		...
		show_seatnum = new JLabel();
		con_seat.add(show_seatnum);
	...
	private class SeatCheckBoxListener implements ActionListener {
	    ...
	    public void actionPerformed(ActionEvent e) {
	        ...
	        // 선택된 좌석 목록을 show_seatnum 라벨에 붙이기
	        ArrayList<String> selectedSeats = test.getShowSeatnum();
	        String labelText = "선택된 좌석: ";
	      ...
	      labelText = labelText.substring(0, labelText.length() - 2);
	        ...
	        // show_seatnum 라벨에 선택된 좌석 목록 설정
	        show_seatnum.setText(labelText);
	    }
	}
